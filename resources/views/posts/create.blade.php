...
<div class="md:flex md:items-center">
    <div class="px-10 md:w-1/2">
        <form action="{{ route('imagenes.store') }}" method="POST" enctype="multipart/form-data" id="dropzone"
            class="flex flex-col items-center justify-center w-full border-2 border-dashed rounded dropzone h-96 border-pet-acento bg-pet-crema">
            @csrf
        </form>
    </div>

    <div class="p-10 mt-10 rounded-lg shadow-xl bg-pet-fondoTarjeta md:w-1/2 md:mt-0">
        ...
        <form id="postForm" action="{{ route('posts.store') }}" method="POST" novalidate>
            @csrf
            <div class="mb-5">
                <label for="titulo" class="block mb-2 font-bold uppercase text-pet-marron">Título</label>
                <input id="titulo" name="titulo" type="text" placeholder="Título de la Publicación"
                    class="w-full p-3 border rounded-lg border-pet-crema bg-pet-beige text-pet-marron" value="{{ old('titulo') }}">
            </div>
            <div class="mb-5">
                <label for="descripcion" class="block mb-2 font-bold uppercase text-pet-marron">Descripción</label>
                <textarea id="descripcion" name="descripcion" placeholder="Descripción de la Publicación"
                    class="w-full p-3 border rounded-lg border-pet-crema bg-pet-beige text-pet-marron">{{ old('descripcion') }}</textarea>
            </div>
            <div class="mb-5">
