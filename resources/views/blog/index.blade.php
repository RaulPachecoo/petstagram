...
<div class="px-4">
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        @foreach($entries as $entry)
            <article class="p-4 transition duration-300 border shadow-md bg-pet-fondoTarjeta border-pet-crema rounded-2xl hover:shadow-lg">
                @if($entry->image)
                    <img src="{{ asset('entries/' . $entry->image) }}" alt="{{ $entry->title }}"
                         class="object-cover w-full h-48 mb-4 rounded-xl">
                @endif
                <h2 class="mb-2 text-xl font-bold text-pet-marron">{{ $entry->title }}</h2>
                <p class="mb-3 text-pet-marron">{{ Str::limit($entry->content, 150) }}</p>
